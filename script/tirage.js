...
function createTirage() {
    ...
    var tirageContainer = document.getElementsByClassName("box-container")[0];
...
function startTirage() {
    ...
    setTimeout(function () {
        ...
        var winnerBox = boxes[winnerNumber - 1];
        winnerBox.classList.remove("highlight");

        // Supprimer toutes les autres boxes
        for (var i = boxes.length - 1; i >= 0; i--) {
            if (i !== winnerNumber - 1) {
                boxes[i].remove();
            }
        ...
        // Centrer la box gagnante
        winnerBox.style.transition = "transform 8s ease-in-out";
        winnerBox.style.transform = "scale(3.3)";
        winnerBox.style.transformOrigin = "top";

        // Jouer le son "gagnant"
        var gagnantSound = document.getElementById("gagnantSound");
        gagnantSound.volume = 0.2;
        gagnantSound.play();
    }, 6000);
}
...
function loop() {
    ...
    if (timerTick >= timerTotal) {
        if (!mousedown) {
            // start the firework at the middle center of the screen, then set the random target coordinates, the random y coordinates will be set within the range of the top half of the screen
            fireworks.push(
