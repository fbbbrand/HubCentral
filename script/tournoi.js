...
function trierParMultiplicateur() {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    document.getElementById("pseudoGagnant" + (i + 1)).textContent =
      nomParticipant;
  }
}
...
function determinerGagnant() {
  ...
  var multi1 = parseFloat(
    document.getElementById("gain1").querySelector(".gain").value
  );
  var multi2 = parseFloat(
    document.getElementById("gain2").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi1 > multi2
      ? document.getElementById("pseudoGagnant1").textContent
      : document.getElementById("pseudoGagnant2").textContent;
}
...
function determinerGagnant1() {
  // Extraction des valeurs des éléments multi
  var multi3 = parseFloat(
    document.getElementById("gain3").querySelector(".gain").value
  );
  var multi4 = parseFloat(
    document.getElementById("gain4").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi3 > multi4
      ? document.getElementById("pseudoGagnant3").textContent
      : document.getElementById("pseudoGagnant4").textContent;
}
...
function determinerGagnant2() {
  // Extraction des valeurs des éléments multi
  var multi5 = parseFloat(
    document.getElementById("gain5").querySelector(".gain").value
  );
  var multi6 = parseFloat(
    document.getElementById("gain6").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi5 > multi6
      ? document.getElementById("pseudoGagnant5").textContent
      : document.getElementById("pseudoGagnant6").textContent;
}
...
function determinerGagnant3() {
  // Extraction des valeurs des éléments multi
  var multi7 = parseFloat(
    document.getElementById("gain7").querySelector(".gain").value
  );
  var multi8 = parseFloat(
    document.getElementById("gain8").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi7 > multi8
      ? document.getElementById("pseudoGagnant7").textContent
      : document.getElementById("pseudoGagnant8").textContent;
}
...
function determinerGagnant4() {
  // Extraction des valeurs des éléments multi
  var multi9 = parseFloat(
    document.getElementById("gain9").querySelector(".gain").value
  );
  var multi11 = parseFloat(
    document.getElementById("gain11").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi9 > multi11
      ? document.getElementById("pseudoGagnant9").textContent
      : document.getElementById("pseudoGagnant11").textContent;
}
...
function determinerGagnant5() {
  // Extraction des valeurs des éléments multi
  var multi10 = parseFloat(
    document.getElementById("gain10").querySelector(".gain").value
  );
  var multi12 = parseFloat(
    document.getElementById("gain12").querySelector(".gain").value
  );
  ...
  pseudoGagnant.textContent =
    multi10 > multi12
      ? document.getElementById("pseudoGagnant10").textContent
      : document.getElementById("pseudoGagnant12").textContent;
}
...
function Vainqueur() {
  ...
  var multi13 = parseFloat(
    document.getElementById("gain13").querySelector(".gain").value
  );
  var multi14 = parseFloat(
    document.getElementById("gain14").querySelector(".gain").value
  );

  var gagnant =
    multi13 > multi14
      ? document.getElementById("pseudoGagnant13").textContent
      : document.getElementById("pseudoGagnant14").textContent;
...
document
  .querySelectorAll(
    ".colonneGain input, .colonnePseudo input, .colonneSlot input"
  )
...
document
  .getElementById("boutonReset")
  .addEventListener("click", effacerContenuInputs);



document.addEventListener('DOMContentLoaded', function() {
            function updateColors() {
                const rows = document.querySelectorAll('.ligne');
                const gains = [];

                rows.forEach(row => {
                    const gainInput = row.querySelector('.gain-input');
                    
                    // Vérifiez si gainInput existe
                    if (gainInput) {
                        // Extraire les valeurs des champs de gains
                        const gainValue = parseFloat(gainInput.value);
                        if (!isNaN(gainValue)) { // Assurez-vous que la valeur est un nombre valide
                            const lineNumber = row.getAttribute('data-line');
                            gains.push({ row, gainValue, lineNumber });
                            console.log(`Gain pour la ligne ${lineNumber}: ${gainValue}`);
                        }
                    }
                });

                // Trier les gains en ordre décroissant
                gains.sort((a, b) => b.gainValue - a.gainValue);
                console.log('Gains triés:', gains);

                // Réinitialiser la couleur des lignes
                rows.forEach(row => row.classList.remove('highlight'));

                // Mettre en surbrillance la ligne avec le 8e nombre le plus grand
                if (gains.length >= 8) {
                    const lineNumber = gains[7].lineNumber;
                    console.log(`Ligne avec le 8e nombre le plus grand: ${lineNumber}`);
                    const rowToHighlight = document.querySelector(`.ligne[data-line="${lineNumber}"]`);
                    if (rowToHighlight) {
                        rowToHighlight.classList.add('highlight');
                        console.log(`Ligne ${lineNumber} surlignée.`);
                    } else {
                        console.log(`Aucune ligne trouvée avec data-line="${lineNumber}".`);
                    }
                } else {
                    console.log('Il y a moins de 8 lignes avec des gains valides.');
                }
            }

            // Attacher l'événement input à chaque champ gain-input
